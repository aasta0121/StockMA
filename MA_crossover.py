...
def MovingAverageCrossStrategy(stock_symbol = 'ULTRACEMCO.NS', start_date = '2018-01-01', end_date = '2020-01-01', 
                               short_window = 5, middle_window = 10, long_window = 20, moving_avg = 'SMA', display_table = True):
    '''
    ...
    middle_window_col = str(middle_window) + '_' + moving_avg  
    ...
    if moving_avg == 'SMA':
        ...
        # Create a middle simple moving average column
        stock_df[middle_window_col] = stock_df['Close Price'].rolling(window = middle_window, min_periods = 1).mean()
        ...
        stock_df[middle_window_col] = stock_df['Close Price'].ewm(span = middle_window, adjust = False).mean()
    ...
    stock_df[middle_window_col].plot(color = 'y', lw = 1, label = middle_window_col) 
...
MovingAverageCrossStrategy('2330.TW', '2021-01-01', '2021-12-22', 5, 10, 20, 'EMA', display_table = True)